...
<script>
  $(function() {
    ...
    $('#mitra-table').on('click', '.delete', function() {
      let token = $("meta[name='csrf-token']").attr("content");
      var id = $(this).data('id');
      $('#formDelete').modal('show');
      $('#delete-post').click(function(e) {
        $.ajax({
            url: "/laporan/mitra/delete/" + id,
            type: 'DELETE',
            data: {
              "id": id,
              "_token": token,
    ...
    $('#mitra-table').on('click', '.update', function() {
      var id = $(this).data('id');
      window.location.href = '/laporan/mitra/update/'+id;
    });
...
<div class="navbar-nav-right d-flex align-items-center">
  <div class="demo-inline-spacing pb-1 mb-4">
    <a href="{{ route('laporan-mitra.add') }}" type="button" class="btn" style="background-color: #0a6ae7; color: #ffffff;">DATA BARU</a>
  ...
  <div class="ms-auto">
    @can('isAdmin')
    <form id="form" class="mb-0" action="{{ route('laporan-mitra') }}" method="GET">
      @csrf
...
<div class="row mb-5">
  <div class="col-md-6 col-lg-12">
    <div class="card mb-3">
      <div class="card-body">
        <h5 class="card-title">KERJASAMA MITRA &nbsp;&nbsp;<small class="badge bg-dark">@auth {{auth()->user()->nama_lengkap}} @endauth</small></h5>
        <p class="text-muted">Kumpulan seluruh data kerjasama mitra yang telah di daftarkan ke dalam sistem.</p>
      </div>
      <div class="card-body">
        {{ $dataTable->table() }}
      </div>
