...
<script>
  $(function() {
    ...
    $('#quotation-table').on('click', '.delete', function() {
      let token = $("meta[name='csrf-token']").attr("content");
      var id = $(this).data('id');
      $('#formDelete').modal('show');
      $('#delete-post').click(function(e) {
        $.ajax({
            url: "/laporan/quotation/delete/" + id,
            type: 'DELETE',
            data: {
              "id": id,
              "_token": token,
    ...
    $('#quotation-table').on('click', '.update', function() {
      var id = $(this).data('id');
      window.location.href = '/laporan/quotation/update/'+id;
    });

    $('#quotation-table').on('click', '.download', function () {
      var fileName = $(this).data('file');
      var downloadUrl = "{{ route('laporan-quotation.download', ['fileName' => ':fileName']) }}".replace(':fileName', fileName);
...
<div class="navbar-nav-right d-flex align-items-center">
  <div class="demo-inline-spacing pb-1 mb-4">
    <a href="{{ route('laporan-quotation.add') }}" type="button" class="btn" style="background-color: #0a6ae7; color: #ffffff;">DATA BARU</a>
  ...
  <div class="ms-auto">
    @can('isAdmin')
    <form id="form" class="mb-0" action="{{ route('laporan-quotation') }}" method="GET">
      @csrf
...
<div class="row mb-5">
  <div class="col-md-6 col-lg-12">
    <div class="card mb-3">
      <div class="card-body">
        <h5 class="card-title">QUOTATION &nbsp;&nbsp;<small class="badge bg-dark">@auth {{auth()->user()->nama_lengkap}} @endauth</small>&nbsp;&nbsp;<small class="badge bg-warning">{{ 'Rp. '.number_format($totalAkumulasi, 0, ',', '.'); }}</small></h5>
        <p class="text-muted">Kumpulan seluruh data quotation yang telah di daftarkan ke dalam sistem.</p>
      </div>
      <div class="card-body">
